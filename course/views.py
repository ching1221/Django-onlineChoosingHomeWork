...
def collect(request):
    ...
    if request.POST['stuid1'] or request.POST['stuid2'] or request.POST['stuid3']:
        if request.POST['projectid']:
            # 题目是否自选判断
            if int(request.POST['projectid']) is not 0:
                SelectP = Project.objects.get(id=int(request.POST['projectid']))
            else:
                if request.POST['pro-name'] and request.POST['pro-desc']:
                    SelectP = Project.objects.create(Name=request.POST['pro-name'], Desc=request.POST['pro-desc'], MaxNum=0)
                    SelfLabel = 1
                else:
                    return HttpResponse("输入的自拟题目信息为空，请仔细核对！"+"<br/>"+"<a href=\"course/ \">返回选题</a>")
...
def homepage(request):
    ...
    Pro_List = Project.objects.filter(MaxNum__gt=0)
